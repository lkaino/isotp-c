...
static int isotp_send_flow_control(IsoTpLink *link, uint8_t flow_status, uint8_t block_size, uint8_t st_min_ms)
{
    ...
    ret = isotp_user_send_can(link->user_ctx, link->send_arbitration_id, message.as.data_array.ptr, sizeof(message));
#else
    ret = isotp_user_send_can(link->user_ctx, link->send_arbitration_id,
...
static int isotp_send_single_frame(IsoTpLink *link, uint32_t id)
{
    ...
    ret = isotp_user_send_can(link->user_ctx, id, message.as.data_array.ptr, sizeof(message));
#else
    ret = isotp_user_send_can(link->user_ctx, id,
                              message.as.data_array.ptr,
                              link->send_size + 1);
#endif
...
static int isotp_receive_single_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len)
{
    ...
    {
        isotp_user_debug(link->user_ctx, "Single-frame length too small.");
...
static int isotp_receive_first_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len)
{
    ...
    if (8 != len)
    {
        isotp_user_debug(link->user_ctx, "First frame should be 8 bytes in length.");
        return ISOTP_RET_LENGTH;
    }
    ...
    if (payload_length <= 7)
    {
        isotp_user_debug(link->user_ctx, "Should not use multiple frame transmission.");
        return ISOTP_RET_LENGTH;
    }

    if (payload_length > link->receive_buf_size)
    {
        isotp_user_debug(link->user_ctx, "Multi-frame response too large for receiving buffer.");
...
static int isotp_receive_consecutive_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len)
{
    ...
    {
        isotp_user_debug(link->user_ctx, "Consecutive frame too short.");
        return ISOTP_RET_LENGTH;
    }
...
static int isotp_receive_flow_control_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len)
{
    ...
    if (len < 3)
    {
        isotp_user_debug(link->user_ctx, "Flow control frame too short.");
        return ISOTP_RET_LENGTH;
    }
...
int isotp_send_with_id(IsoTpLink *link, uint32_t id, const uint8_t payload[], uint16_t size)
{
    ...
    if (link == 0x0)
    {
        isotp_user_debug(link->user_ctx, "Link is null!");
        return ISOTP_RET_ERROR;
    }

    if (size > link->send_buf_size)
    {
        isotp_user_debug(link->user_ctx, "Message size too large. Increase ISO_TP_MAX_MESSAGE_SIZE to set a larger buffer\n");
        char message[128];
    ...
    {
        isotp_user_debug(link->user_ctx, "Abort previous message, transmission in progress.\n");
...
void isotp_init_link(IsoTpLink *link, uint32_t sendid,
                     ...
                     uint8_t *recvbuf, uint16_t recvbufsize, void *user_ctx)
{
